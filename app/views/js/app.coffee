...
comment_show = (json, father) ->
  ...
  if comment.success
    ...
    voting.append $("<div>",
      class: "arrow sprite upmod"
      onclick: "vote(event, '" + comment.hash + "');"
    )
    voting.append $("<div>",
      class: "arrow sprite down"
      onclick: "vote(event, '" + comment.hash + "');"
    )
    detail = $("<div>",
      class: "comment_detail entry"
    )
    ac_karma = $("<div>",
      class: "action"
    ).append($("<i>",
    ...
    ac_time = $("<div>",
      ...
      class: "icon-time"
    )).append(" ").append(comment.created_at).append(" ")
    ac_link = $("<div>",
      class: "action"
    ).append($("<i>",
      class: "icon-link"
    )).append(" ").append($("<a>",
      href: "/p/" + comment.hash.replace("_", "#")
      text: "链接"
    )).append(" ")
    ac_reply = $("<div>",
      class: "action"
    ).append($("<a>",
      onclick: "comment_reply(event, '" + comment.hash + "');"
      html: "<i class='icon-comment-alt'></i> 回复 "
    ))
    ac_edit = $("<div>",
      class: "action"
    ).append($("<a>",
      onclick: "comment_edit(event, '" + comment.hash + "');"
      html: "<i class='icon-edit'></i> 编辑 "
    ))
    ac_children = $("<div>",
      class: "action"
    ).append($("<i>",
      class: "icon-cut"
    )).append(" ").append("0个子评论").append(" ")
    detail.append $("<div>",
      class: "tagline"
    ).append($("<a>",
      href: "javascript:void(0);"
      onclick: "hide_comment(event)"
      html: "[-] "
    )).append($("<strong>").append($("<a>",
      href: "/u/" + logged
      html: logged
    ))).append(ac_karma).append(ac_time).append(ac_link).append(ac_reply).append(ac_edit)
    detail.append $("<div>",
      class: "md current_user"
      html: comment.rendered_content
    )
    detail.append $("<div>",
      class: "child"
    ).append($("<div>",
    ...
    show = $("<div>",
      class: "comment " + comment.id_hash
      id: comment.hash.split("_").pop()
    )
    show.append voting
    show.append detail
    hide = $("<div>",
      class: "comment_hide " + comment.id_hash
      style: "display: none"
    )
    hide.append($("<a>",
      href: "javascript:void(0);"
      onclick: "show_comment(event);"
      text: "[+] "
    )).append($("<strong>").append($("<a>",
      href: "/u/" + logged
      html: logged
    ))).append(ac_karma.clone()).append(ac_time.clone()).append ac_children
    clear = $("<div>",
      class: "clearleft"
    )
